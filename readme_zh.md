...
**v3.5.1 (Recycle Bin Fixes)**
*   **Fix (Frontend)**: 修复了将项目移入回收站时，仅更新前端 UI 而未调用后端 API 的严重 Bug。现在项目能正确进入回收站。
*   **Fix (frontend)**: 修复了从回收站恢复项目后，"我的作品"列表未实时刷新的问题。
*   **Feature**: 确认了后端已包含自动清理超过30天项目的逻辑 (On Startup)。

**v3.5.2 (Default Mind Maps)**
*   **Feature (Backend)**: 优化项目初始化逻辑 (`/api/projects/from-card`)。
*   **Update**: 现在创建新项目时，会默认生成两张标准的空白思维导图，并预置以下结构：
    *   **核心架构**: 世界设定、爽点体系、角色塑造、关键角色成长体系。
    *   **事件发展**: 分卷大纲、事件流、单元细纲、章节细纲、角色状态时间线、剧情锚点伏笔提取。

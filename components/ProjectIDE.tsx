
import React, { useEffect, useState } from 'react';
import { Project, ProjectStructure, Chapter, MindMap } from '../types';
import { apiService } from '../services/geminiService';
import { logger } from '../services/loggerService';

interface Props {
    project: Project;
    onBack: () => void;
}

type FileType = 'chapter' | 'mindmap';

export const ProjectIDE: React.FC<Props> = ({ project, onBack }) => {
    const [structure, setStructure] = useState<ProjectStructure>({ chapters: [], maps: [] });
    const [loading, setLoading] = useState(true);
    const [activeFile, setActiveFile] = useState<{ type: FileType, id: string, title: string } | null>(null);

    // 加载项目结构
    useEffect(() => {
        const loadStructure = async () => {
            try {
                const data = await apiService.getProjectStructure(project.id);
                setStructure(data);
                // 默认选中第一个思维导图
                if (data.maps.length > 0) {
                    setActiveFile({ type: 'mindmap', id: data.maps[0].id, title: data.maps[0].title });
                } else if (data.chapters.length > 0) {
                    setActiveFile({ type: 'chapter', id: data.chapters[0].id, title: data.chapters[0].title });
                }
            } catch (e) {
                logger.error("加载项目结构失败", e);
            } finally {
                setLoading(false);
            }
        };
        loadStructure();
    }, [project.id]);

    const handleFileClick = (type: FileType, id: string, title: string) => {
        setActiveFile({ type, id, title });
    };

    return (
        <div className="fixed inset-0 z-40 bg-dark flex flex-col text-slate-200">
            {/* Top Bar */}
            <div className="h-12 border-b border-slate-700 bg-slate-900 flex items-center px-4 justify-between flex-shrink-0">
                <div className="flex items-center gap-3">
                    <button onClick={onBack} className="text-slate-400 hover:text-white transition-colors" title="返回首页">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    </button>
                    <span className="font-bold text-indigo-400">SkyCraft IDE</span>
                    <span className="text-slate-600">/</span>
                    <span className="text-sm text-slate-300">{project.title}</span>
                </div>
                <div className="text-xs text-slate-500">
                    环境：纯净模式 (Read Only Preview)
                </div>
            </div>

            <div className="flex-1 flex overflow-hidden">
                {/* Sidebar: File Tree */}
                <div className="w-64 bg-[#0d1117] border-r border-slate-700 flex flex-col flex-shrink-0">
                    <div className="p-3 text-xs font-bold text-slate-500 uppercase tracking-wider">资源管理器</div>
                    
                    {loading ? (
                        <div className="p-4 text-center text-slate-600 text-xs">加载中...</div>
                    ) : (
                        <div className="flex-1 overflow-y-auto space-y-4 p-2">
                            {/* Mind Maps Folder */}
                            <div>
                                <div className="flex items-center gap-1 text-pink-400 text-sm font-bold px-2 py-1 mb-1">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                                    思维导图 (Mind Maps)
                                </div>
                                <div className="pl-4 space-y-1">
                                    {structure.maps.map(map => (
                                        <div 
                                            key={map.id} 
                                            onClick={() => handleFileClick('mindmap', map.id, map.title)}
                                            className={`cursor-pointer px-2 py-1 rounded text-xs truncate transition-colors ${activeFile?.id === map.id ? 'bg-pink-900/30 text-pink-200' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                                        >
                                            {map.title}.json
                                        </div>
                                    ))}
                                    {structure.maps.length === 0 && <div className="text-[10px] text-slate-600 italic px-2">为空</div>}
                                </div>
                            </div>

                            {/* Chapters Folder */}
                            <div>
                                <div className="flex items-center gap-1 text-indigo-400 text-sm font-bold px-2 py-1 mb-1">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                    正文卷宗 (Chapters)
                                </div>
                                <div className="pl-4 space-y-1">
                                    {structure.chapters.map(chap => (
                                        <div 
                                            key={chap.id} 
                                            onClick={() => handleFileClick('chapter', chap.id, chap.title)}
                                            className={`cursor-pointer px-2 py-1 rounded text-xs truncate transition-colors ${activeFile?.id === chap.id ? 'bg-indigo-900/30 text-indigo-200' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                                        >
                                            {chap.title}.txt
                                        </div>
                                    ))}
                                    {structure.chapters.length === 0 && <div className="text-[10px] text-slate-600 italic px-2">为空</div>}
                                </div>
                            </div>
                        </div>
                    )}
                </div>

                {/* Main Editor Area */}
                <div className="flex-1 bg-[#1e1e1e] flex flex-col relative">
                    {/* Tab Bar */}
                    <div className="h-9 bg-[#2d2d2d] flex items-center px-2 space-x-1 overflow-x-auto border-b border-black/50">
                        {activeFile && (
                            <div className="bg-[#1e1e1e] text-xs px-3 py-1.5 min-w-[100px] border-t-2 border-primary text-slate-200 flex items-center justify-between gap-2">
                                <span>{activeFile.title}</span>
                                <span className="hover:text-red-400 cursor-pointer">×</span>
                            </div>
                        )}
                    </div>

                    {/* Content */}
                    <div className="flex-1 p-8 overflow-y-auto">
                        {activeFile ? (
                            <div className="max-w-3xl mx-auto animate-fade-in">
                                {activeFile.type === 'chapter' && (
                                    <div className="prose prose-invert prose-slate">
                                        <h1 className="text-3xl font-bold mb-6 text-slate-100">{activeFile.title}</h1>
                                        <div className="text-slate-500 italic border-l-4 border-indigo-600 pl-4 py-2 bg-indigo-900/10">
                                            [系统提示] 此处为正文编辑区。当前内容为空，等待 AI 生成或人工撰写。
                                        </div>
                                        <div className="mt-8 h-96 border border-dashed border-slate-700 rounded-lg flex items-center justify-center text-slate-600">
                                            正文编辑器占位符 (Editor Placeholder)
                                        </div>
                                    </div>
                                )}
                                {activeFile.type === 'mindmap' && (
                                    <div className="h-full flex flex-col">
                                        <h2 className="text-xl font-bold mb-4 text-pink-400">{activeFile.title} - 架构图</h2>
                                        <div className="flex-1 bg-slate-900/50 border border-slate-700 rounded-lg h-[600px] flex items-center justify-center relative overflow-hidden group">
                                            <div className="absolute inset-0 grid grid-cols-[40px_40px] opacity-10" style={{backgroundImage: 'linear-gradient(to right, #444 1px, transparent 1px), linear-gradient(to bottom, #444 1px, transparent 1px)', backgroundSize: '40px 40px'}}></div>
                                            <div className="text-center">
                                                <div className="w-24 h-12 bg-pink-600 rounded flex items-center justify-center text-white font-bold mb-4 mx-auto shadow-lg shadow-pink-500/20">核心创意</div>
                                                <div className="w-0.5 h-8 bg-slate-600 mx-auto"></div>
                                                <div className="flex gap-8 mt-2">
                                                    <div className="border border-slate-600 px-3 py-1 rounded text-xs text-slate-400">人设架构</div>
                                                    <div className="border border-slate-600 px-3 py-1 rounded text-xs text-slate-400">剧情主线</div>
                                                </div>
                                            </div>
                                            <div className="absolute bottom-4 right-4 text-xs text-slate-600">
                                                Mind Map Canvas Preview
                                            </div>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="h-full flex flex-col items-center justify-center text-slate-600">
                                <svg className="w-24 h-24 mb-4 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                                <p>请在左侧选择文件开始编辑</p>
                            </div>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
};
